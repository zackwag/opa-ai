...
export function generateAppealPdf(subject, selectedComps, salesData) {
    ...
    const subjectLines = [
        ...
        `Livable Area: ${subject.total_livable_area ? Number.parseInt(subject.total_livable_area, 10).toLocaleString() + ' sqft' : 'N/A'}  |  Lot Area: ${subject.total_area ? Number.parseInt(subject.total_area, 10).toLocaleString() + ' sqft' : 'N/A'}`,
    ...
    const compValues = selectedComps.map(c => Number.parseInt(c.market_value, 10)).filter(v => !Number.isNaN(v));
    ...
    selectedComps.forEach((comp) => {
        ...
        const row = [
            ...
            comp.total_livable_area ? Number.parseInt(comp.total_livable_area, 10).toLocaleString() : 'N/A',
            comp.year_built || 'N/A',
            distText,
        ];
    ...
    if (salesData && salesData.length > 0) {
        ...
        const belowCount = salesData.filter(s => Number.parseInt(s.sale_price, 10) < subjectValue).length;
        ...
        displaySales.forEach(sale => {
            ...
            const row = [
                ...
                sale.total_livable_area ? Number.parseInt(sale.total_livable_area, 10).toLocaleString() : 'N/A',
                distText,
            ];
