...
import { computeAppealStats, computeSalesStats, findLowerComps, findRecentSales, normalizeAddress, scoreSimilarity, searchProperty } from './api'
import './App.css'
        ...
        <div className="subject-card">
            ...
            <div className="property-grid">
                ...
                <div className="property-stat">
                    ...
                    <span className="value">{property.total_livable_area ? `${Number.parseInt(property.total_livable_area, 10).toLocaleString()} sqft` : 'N/A'}</span>
                </div>
                <div className="property-stat">
                ...
                <div className="property-stat">
                    ...
                    <span className="value">{property.total_area ? `${Number.parseInt(property.total_area, 10).toLocaleString()} sqft` : 'N/A'}</span>
                </div>
                <div className="property-stat">
...
function CompCard({ comp, subjectValue, selected, onToggle, selectionFull }) {
    const savings = Number.parseInt(subjectValue, 10) - Number.parseInt(comp.market_value, 10);
    let distanceText = 'N/A';
    if (comp.distance_m != null) {
        distanceText = comp.distance_m < 100
            ? `${Number.parseInt(comp.distance_m, 10).toLocaleString()}m`
            : `${(Number.parseFloat(comp.distance_m) / 1000).toFixed(2)}km`;
    }
    let selectTitle = 'Select this comparable';
    if (selected) {
        selectTitle = 'Deselect this comparable';
    } else if (selectionFull) {
        selectTitle = 'Selection full';
    }
    ...
    return (
        <div className={`comp-card ${selected ? 'comp-selected' : ''}`}>
            <div className="comp-card-header">
                ...
                <button type="button" className={`select-btn ${selected ? 'selected' : ''}`} onClick={onToggle} disabled={!selected && selectionFull} title={selectTitle}>
                    {selected ? '✓' : '+'}
                </button>
            </div>
            <div className="details">
                ...
                <div className="detail">
                    <FieldLabel text="Livable Area" />
                    <span className="value">{comp.total_livable_area ? `${Number.parseInt(comp.total_livable_area, 10).toLocaleString()} sqft` : 'N/A'}</span>
                </div>
                <div className="detail">
                    <FieldLabel text="Year Built" />
...
function SaleCard({ sale, subjectValue }) {
    const diff = Number.parseInt(sale.sale_price, 10) - Number.parseInt(subjectValue, 10);
    const below = diff < 0;
    let distanceText = 'N/A';
    if (sale.distance_m != null) {
        distanceText = sale.distance_m < 100
            ? `${Math.round(sale.distance_m)}m`
            : `${(sale.distance_m / 1000).toFixed(2)}km`;
    }
    ...
    return (
        <div className={`comp-card ${below ? 'sale-below' : ''}`}>
            ...
            <div className="details">
                ...
                <div className="detail">
                    <FieldLabel text="Livable Area" />
                    <span className="value">{sale.total_livable_area ? `${Number.parseInt(sale.total_livable_area, 10).toLocaleString()} sqft` : 'N/A'}</span>
                </div>
                <div className="detail">
                    <FieldLabel text="Year Built" />
...
function App() {
    ...
    function toggleComp(comp) {
        setSelectedComps(prev => {
            const exists = prev.some(c => c.parcel_number === comp.parcel_number);
            if (exists) {
                return prev.filter(c => c.parcel_number !== comp.parcel_number);
            }
            if (prev.length >= 5) {
                return prev;
            }
        ...
        <>
            ...
            <section className="search-section">
                <form className="search-form" onSubmit={handleSearch}>
                    <div className="field">
                        ...
                        <AddressAutocomplete value={address} onSelect={(addr) => { setAddress(addr); doSearch(addr); }} onChange={(val) => setAddress(val)} disabled={loading} />
                    </div>
            ...
            {searchResults && (
                <section className="search-section">
                    ...
                    {searchResults.map(r => (
                        <button type="button" key={r.parcel_number} className="search-result-row" onClick={() => selectProperty(r)}>
            ...
            {selectedComps.length > 0 && (
                <div className="pdf-bar">
                    ...
                    <button type="button" className="pdf-btn" onClick={() => generateAppealPdf(subject, selectedComps, recentSales)}>
                        Generate Appeal PDF
                    </button>
                </div>
            )}
            ...
            {comps && comps.length > 0 && (
                <section className="comps-section">
                    ...
                    <div className="comps-grid">
                        {comps.map(comp => (
                            <CompCard key={comp.parcel_number} comp={comp} subjectValue={subject.market_value} selected={isSelected(comp)} onToggle={() => toggleComp(comp)} selectionFull={selectedComps.length >= 5} />
                        ))}
                    </div>
                </section>
            )}
